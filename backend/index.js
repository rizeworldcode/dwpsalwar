...
app.use((req, res, next) => {
  ...
  res.setHeader(
    ...
    "frame-ancestors 'self' http://localhost:5173 http://localhost:5174 http://localhost:5175 http://localhost:5176 http://localhost:5177 http://localhost:5178 http://localhost:5179 http://localhost:5180"
  );
...
app.use('/', require('./src/routes/TC_uplode.js'));
app.use('/', require('./src/routes/admin_dashbord.js'));
app.use('/', require('./src/routes/student_validation.js'));
...
server.listen(process.env.PORT || 50000, () => {
  console.log(`Server is running on http://localhost:${process.env.PORT || 50000}`);
});
